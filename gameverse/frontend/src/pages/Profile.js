...
const Profile = () => {
  ...
  const shareProfile = () => {
    ...
    alert('Enllaç de perfil copiat! Comparteix-lo amb els teus amics 🎮');
  };
  ...
  const joinDate = new Date(user.created_at).toLocaleDateString('ca-ES', { year: 'numeric', month: 'long' });
          ...
          <div className="profile-hero-inner">
            ...
            <div className="profile-hero-info">
              ...
              <div className="profile-meta">
                ...
                <span>❤️ {favorites.length} favorits</span>
                {avgRating && <span>⭐ Puntuació mitjana: {avgRating}/10</span>}
              </div>
            </div>
            <button className="btn btn-secondary" onClick={shareProfile}>🔗 Compartir perfil</button>
          </div>
        </div>
      </div>
              ...
              {favorites.map(f => (
                <Link key={f.id} to={`/game/${f.game_id}`} className="profile-fav-card card">
                  {f.game_background_image ? (
                    <img src={f.game_background_image} alt={f.game_name} className="profile-fav-img" loading="lazy" />
                  ) : (
                    <div className="profile-fav-no-img">🎮</div>
                  )}
                  <div className="profile-fav-info">
                    <span className="profile-fav-name">{f.game_name}</span>
                    {f.user_rating && <span className="profile-fav-rating">⭐ {f.user_rating}/10</span>}
                  </div>
                </Link>
              ))}
            </div>
